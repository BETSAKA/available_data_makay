...
## A focus on the Androy protected area

We download the Makay protected area boundaries.
...
tm_shape(makay_union) +
  ...
  tm_borders(col = "blue") +
  tm_fill(alpha = 0, id = "NAME_4", popup.vars = c("NAME_3", "NAME_2", "NAME_1")) +
  tm_basemap("OpenStreetMap")
```
...
# copy chirps locally (from S3 because very heavy)
source("s3_utils.R")
my_bucket <- get_bucket_df("fbedecarrats", region = "")
chirps_files <- my_bucket %>%
  filter(str_starts(Key, "mapme_biodiversity/chirps")) %>%
  pluck("Key")

map2(chirps_files, chirps_files, save_from_s3)

aoi <- init_portfolio(makay_union, years = 1980:2021) %>%
