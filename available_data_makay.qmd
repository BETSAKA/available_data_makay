...
We also load the perimeters of administrative areas

```{r}
# On crée une forme fusionnant les parties de Makay
makay_ap_all <- st_union(makay_ap)

mada <- gadm("MDG", level = 0, path = tempdir()) %>%
  st_as_sf() 
provinces_mada <- gadm("MDG", level = 1, path = tempdir()) %>%
  st_as_sf() 
regions_mada <- gadm("MDG", level = 2, path = tempdir()) %>%
  st_as_sf() 
communes_mada <- gadm("MDG", level  =4, path = tempdir()) %>%
  st_as_sf() 

toliary <- provinces_mada %>%
  filter(NAME_1 == "Toliary")

communes_makay <- st_intersection(communes_mada, makay_ap)
