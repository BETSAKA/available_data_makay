...
We download the Makay protected area boundaries, as well as the borders of the surrounding adminitrative areas.
...
mada <- gadm("MDG", level = 0, path = tempdir()) %>%
  st_as_sf() 
provinces_mada <- gadm("MDG", level = 2, path = tempdir()) %>%
  st_as_sf() 
regions_mada <- gadm("MDG", level = 3, path = tempdir()) %>%
  st_as_sf() 
