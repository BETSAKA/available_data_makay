...
download_firms_data <- function(token, target_directory) {
  ...
  for (date in dates) {
    formatted_date <- format(date, "%Y.%m.%d") # Define this outside of tryCatch
    ...
    tryCatch({
      ...
      if (!file.exists(target_path)) {
        ...
        if (http_status(response)$category == "Success") {
          ...
          cat("Downloaded to:", target_path, "\n")
        } else {
          cat("Failed to download for date:", formatted_date, ". Status:", http_status(response)$message, "\n")
        }
      } else {
        cat("File for date", formatted_date, "already exists. Skipping download.", "\n")
      }
    }, error = function(e) {
      cat("Error on date:", formatted_date, ". Message:", e$message, "\n")
    })
  }
}
...
if (Sys.info()["effective_user"] == "onyxia") {
  ...
  my_secret <- readLines("secret_api_key")
}
