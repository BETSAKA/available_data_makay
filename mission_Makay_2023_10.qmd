...
On veut aussi observer des événements de déforestation qui auraient eu lieu récemment. On mobilise pour cela le jeu de données GFC. The Global Forest Cover dataset is derived from Landsat observations, to map forest extents and track changes over time at a global scale with a 30-meter resolution (Hansen et al. 2013). This dataset distinguishes between forest cover in 2000 and losses occurred each year from 2001 to 2022. It is a practical tool that has been widely adopted to evaluate the impact of protected areas on forest loss. However, the Global Forest Cover dataset has limitations, including potential misclassifications for atypical forest ecosystem or the challenges of distinguishing between natural forest and tree plantations. These challenges might prove critical for dry biomes where the global forest cover dataset is known for under-estimating forest cover and forest cover loss.  Ci-dessous on présente les données les plus récentes disponibles, en distinguant les perte de couvert forestier survenues avant 2021 et celles survenues en 2021 et 2022 (on n'a pas encore de données pour 2023). 
...
On observe sur ces données les éléments suivants: 
Peu de déforestation relevée en général. PRatiquement pas de points récents, hormis une occurence à 1km au Sud de Tsivoko et plus d'occurrence à une dizaine de kilomètres au Sud Est de Tsivoko.Quelques points plus anciens sont à signaler au Nord de Beronono.
